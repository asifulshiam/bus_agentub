...
def get_owner_dashboard(
    ...
    active_trips = db.query(Bus).filter(
    ...
    total_bookings = db.query(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id
    ).count()
    
    # Get confirmed bookings
    confirmed_bookings = db.query(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id,
        Booking.status == BookingStatus.accepted
    ).count()
    
    # Get total revenue from confirmed tickets
    revenue_result = db.query(func.sum(Ticket.total_fare)).join(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id,
    ...
    today_revenue_result = db.query(func.sum(Ticket.total_fare)).join(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id,
        Ticket.status == TicketStatus.confirmed,
    ...
    pending_bookings = db.query(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id,
...
def get_owner_buses(
    ...
    for bus in buses:
        ...
        boarding_points_count = db.query(BoardingPoint).filter(
            BoardingPoint.bus_id == bus.id
        ).count()
        ...
        if bus.supervisor_id:
            supervisor_user = db.query(User).filter(User.id == bus.supervisor_id).first()
            if supervisor_user:
                supervisor = {
...
def get_ticket_sales_report(
    ...
    query = db.query(Ticket).join(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id,
        Ticket.status == TicketStatus.confirmed
    )
    ...
    if bus_id:
        ...
        bus = db.query(Bus).filter(Bus.id == bus_id, Bus.owner_id == current_user.id).first()
        if not bus:
            raise HTTPException(
...
def get_supervisors(
    ...
    Returns list of all supervisor users for assignment to buses.
    """
    ...
    return [UserResponse.model_validate(supervisor) for supervisor in supervisors]
...
def get_revenue_summary(
    period: str = Query("month", regex="^(day|week|month|year)$", description="Revenue period"),
    current_user: User = Depends(get_current_owner),
    db: Session = Depends(get_db)
):
    """
    ...
    tickets = db.query(Ticket).join(Booking).join(Bus).filter(
        Bus.owner_id == current_user.id,
        Ticket.status == TicketStatus.confirmed,
    ...
    return {
        ...
        "average_ticket_value": total_revenue / total_tickets if total_tickets > 0 else 0.0
    }
