...
from app.schemas.bus import BusPublicResponse
from app.dependencies import get_current_user, get_current_supervisor
...
async def get_eta_to_boarding_point(
    ...
    boarding_point = db.query(BoardingPoint).filter(
        BoardingPoint.id == boarding_point_id,
    ...
    try:
        eta_data = await maps_service.calculate_distance_and_duration(
        ...
        if eta_data:
            ...
            distance_km = maps_service.calculate_distance_simple(
                float(bus.current_lat),
                float(bus.current_lng),
                float(boarding_point.lat),
                float(boarding_point.lng)
...
async def get_bus_route(
    ...
    if not has_access:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access denied to bus route"
        )
    ...
    if boarding_points_data and len(boarding_points_data) >= 2:
        try:
            ...
            waypoints = [
                (bp["lat"], bp["lng"]) 
                for bp in boarding_points_data[1:-1]
            ]
            
            route_data = await maps_service.get_route_directions(
    ...
    return {
        ...
        "current_location": {
            ...
            "last_update": bus.last_location_update.isoformat() if bus.last_location_update else None
        } if bus.current_lat and bus.current_lng else None
    }
